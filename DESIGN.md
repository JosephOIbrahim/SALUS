...
7. A second independent recheck (post-v0.1.2) confirmed the fork copy
   ...
   them is open work).
8. That mission-level fixture landed in 0.3.1: `clip_three` runs over
   an authored replay log where entropy is pinned flat, so R2 and R3
   are the only rules that can fire — and both do, once each. The log
   is committed as a fixture and regenerated byte-identically by
   `tools/make_clip_three.py`, which uses iterated multiplication
   instead of `math.exp` so the bytes hold across platforms.
